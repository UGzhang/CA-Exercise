...
static void usage_msg(void) {
	fprintf(stderr, "Usage: ./jacobi <total allocated memory(GiB)>\n");
	return;
}
...
int main(int argc, char *argv[]){
    ...
    // The ouput format:
	// 	1. uint64_t allocate memory in GiB
   ...
   fprintf(stdout, "%" PRIu64 ",%lf,%" PRIu64 ",%" PRIu64 ",%d,%d\n", 
        allocated_mem, mega_updates_per_sec, 
        actual_runtime_us / 1000, minimal_runtime_ms, Bx, By);
